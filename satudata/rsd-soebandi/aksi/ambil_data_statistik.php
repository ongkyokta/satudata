...
$sqlx = ("SELECT SUM(p.dokter_umum) AS jmldr_umum,
                 ...
                 SUM(p.teknis_kefarmasian) AS jmlfarmasi,
                 SUM(p.s1_psikologi_klinis) AS jmls1psikologi,
                 ...
                 SUM(p.elektromedik) AS jmlelektro,
                 SUM(p.perekam_medis) AS jmlperekam,
                 SUM(p.keteknisian_medis) AS jmlteknis,
                 SUM(p.struktural_manajemen) AS jmlstruktural,
...
$resultx = mysqli_query($dbconn,$sqlx);$row= mysqli_fetch_assoc($resultx);
  ...
  $row_array['jmlnakes']=$row['jmldr_umum']+$row['jmldr_spesialis']+$row['jmldr_gigi']+
                         $row['jmlbidan']+$row['jmlperawat']+$row['jmls1psikologi']+
                         $row['jmlapoteker']+$row['jmlfarmasi']+$row['jmlepi_kesehatan']+
                         $row['jmladm_kesehatan']+$row['jmlsanitaria']+$row['jmlgizi']+
                         $row['jmlanalis']+$row['jmlradiografer']+$row['jmlfisio']+
                         $row['jmlelektro']+$row['jmlperekam']+$row['jmlteknis']+
                         $row['jmlstruktural']+$row['jmlpendukung'];
