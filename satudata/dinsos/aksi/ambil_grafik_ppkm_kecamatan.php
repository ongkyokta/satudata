...
$sqlx = ("SELECT DISTINCT (pekerjaan) AS nmbentuksekolah FROM tbl_api_dinsos WHERE id_kecamatan='".$idarea."'");
		
$resultx = mysqli_query($dbconn,$sqlx);
$data = array();
while($t=mysqli_fetch_array($resultx)){		

	$sqlcount = ("SELECT COUNT(IF(bansos_ppkm='YA',0,null)) AS jumSwasta
		FROM tbl_api_dinsos WHERE pekerjaan = '".$t['nmbentuksekolah']."' AND id_kecamatan='".$idarea."'");
