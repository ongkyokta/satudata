...
$data = array();
	$sqlcount = ("SELECT COUNT(IF(bansos_bnpt='YA',0,null)) AS jumbnpt, COUNT(IF(bansos_pkh='YA',0,null)) AS jumpkh,
		...
		FROM tbl_api_dinsos WHERE id_kecamatan='".$idkecamatan."'");
	$resultcount = mysqli_query($dbconn,$sqlcount);$t= mysqli_fetch_assoc($resultcount);

	$sqlcount2 = ("SELECT SUM(jml_penduduk) AS jumpenduduk FROM tbl_api_dispenduk_kecamatan WHERE id_kecamatan='".$idkecamatan."'");
