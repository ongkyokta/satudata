...
$sqlx = ("SELECT DISTINCT (pekerjaan) AS nmbentuksekolah FROM tbl_api_dinsos WHERE id_kecamatan='".$idkecamatan."'");
 $result = mysqli_query($dbconn,$sqlx);
 $data = array();
 $totaldetail =0;
 while($t=mysqli_fetch_array($result)){
	
	$sqlcount = ("SELECT COUNT(nik) as jumDetSekolah FROM tbl_api_dinsos WHERE pekerjaan = '".$t['nmbentuksekolah']."' AND id_kecamatan='".$idkecamatan."'");
