...
$sqlx = ("SELECT DISTINCT (ancaman_bahaya) AS nmancaman FROM tbl_api_bpbd WHERE id_kecamatan='".$idkecamatan."'");
 $result = mysqli_query($dbconn,$sqlx);
 $data = array();
 $totaldetail =0;
 while($t=mysqli_fetch_array($result)){
	
	$sqlcount = ("SELECT SUM(jml_rawan) as jumDetSekolah FROM tbl_api_bpbd WHERE ancaman_bahaya = '".$t['nmancaman']."' AND id_kecamatan='".$idkecamatan."'");
