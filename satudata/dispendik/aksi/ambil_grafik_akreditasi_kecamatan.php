...
$sqlx = ("SELECT DISTINCT (bentuk_pendidikan) AS nmbentuk FROM tbl_api_dispendik WHERE id_kecamatan='".$idkecamatan."'");
...
while($t=mysqli_fetch_array($resultx)){		
	...
	$sqlcount = ("SELECT COUNT(IF(akreditasi='A',0,null)) AS jumA, COUNT(IF(akreditasi='B',0,null)) AS jumB, COUNT(IF(akreditasi='C',0,null)) AS jumC
			...
			FROM tbl_api_dispendik WHERE bentuk_pendidikan = '".$t['nmbentuk']."' AND id_kecamatan='".$idkecamatan."'");
