...
$sqlx = ("SELECT DISTINCT nm_komoditas FROM tbl_api_disperta WHERE jenis='Perkebunan' AND nm_komoditas LIKE '%kopi%'");
 $result = mysqli_query($dbconn,$sqlx);
 $data = array();
 $totaldetail =0;
 while($t=mysqli_fetch_array($result)){
	
	$sqlcount = ("SELECT SUM(hasil) AS jumDetSekolah FROM tbl_api_disperta WHERE nm_komoditas = '".$t['nm_komoditas']."' AND nm_komoditas LIKE '%kopi%' AND id_kecamatan='".$idkecamatan."'");
