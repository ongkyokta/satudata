...
$sqlx = ("SELECT DISTINCT id_komoditas,nm_komoditas FROM tbl_api_disperta WHERE jenis='Perkebunan'");
 $result = mysqli_query($dbconn,$sqlx);
 $data = array();
 $persentasi = 0;$no = 0;
 while($t=mysqli_fetch_array($result)){
	$no++;
	$sqlcount = ("SELECT SUM(luas_tanam) as jumlstanam ,SUM(luas_panen) as jumlspanen ,SUM(hasil) as jumhasil FROM tbl_api_disperta WHERE nm_komoditas = '".$t['nm_komoditas']."'");
